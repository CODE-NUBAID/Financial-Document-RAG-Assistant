...
Implements the core anti-hallucination RAG prompt and attaches page-level
citations + retrieval relevance scores to every answer.
"""
...
SYSTEM_PROMPT = """You are a Financial Document Assistant analyzing invoices.
...
def retrieve_chunks(db, query: str, k: int = 4):
    ...
    FAISS returns L2 distance — lower means more similar. We convert it
    to a 0-100 'relevance' score so it's readable in the UI, but we label
    it honestly as a retrieval-similarity score, not a calibrated
